...
- **Step 5**: 리드/래그 −7 ~ +7일 상관 분석. 검색·뉴스 시그널이
  가격 수익률·거래량을 며칠 선행/후행하는 "경향"을 히트맵·바차트·테이블로
  제시 (예측 아님).
- **Step 6 (현재)**: 차트 위 이벤트 마커. 검색·뉴스·거래량 스파이크
  (z-score &gt; 2σ) 및 가격 ±5% 급변 시점을 캔들 위에 표기. 이벤트 패널
  항목을 클릭하면 해당 날짜의 OHLCV/검색/뉴스/감성 + 매칭 헤드라인이
  인라인으로 펼쳐지고, 차트는 그 날짜 ±5일 범위로 이동.
